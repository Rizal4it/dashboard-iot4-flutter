...
class BedroomScreenState extends State<BedroomScreen> {
  ...
  void _getLEDStatus() async {
    ...
    setState(() {
      isLedOn = prefs.getBool("Bedroom (LED 5)") ?? false;
    });
  }
  ...
  void _startLEDStatusWatcher() {
    ledStatusTimer = Timer.periodic(const Duration(seconds: 1), (_) async {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      bool currentStatus = prefs.getBool("Bedroom (LED 5)") ?? false;
    ...
    try {
      ...
      if (decoded is List) {
        final lastData = decoded.lastWhere(
          (d) => d["sensor_id"] == (isTemperature ? 1 : 7),
          orElse: () => null,
        );
        if (lastData != null) {
          if (isTemperature) {
          ...
          decoded["sensor_id"] == (isTemperature ? 1 : 7)) {
        if (isTemperature) {
